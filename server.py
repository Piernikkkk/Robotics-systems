...
def receive_command(command: str):
    ...
    user_message =  (f"You are a robot Valley. Current position: {current_position}. Respond to the user {command} with an updated JSON object {robot_target} containing the new position 'x', 'y'. Reply ONLY in a JSON format. example: {{'x': 3, 'y': 4}}")
    ...
    ai_json_text = response.choices[0].message.content # get pure text from AI
...
def update_state(state: dict):
    ...
    return {"message": "State updated successfully", "new_state": robot_state}
